...
from encuestas.apiv2.views import get_survey, ready_survey, get_users_by_filter, get_surveys_by_filter, get_messages, \
    ...
    request_message, user_get_historial, ios_debug, survey_script, viewed_survey

urlpatterns = [

    url(r'^supdate/(?P<string>\w+)/$',ready_survey, name='ready_survey'),
    url(r'^sviewed/(?P<string>\w+)/$',viewed_survey, name='viewed_survey'),
    ...
    url(r'^userRegister/(?P<typ>\w+)/$',user_register, name='user_register'),
